...
fuentes_102a_disponibles = (
    not banco.empty
    and not permanencia.empty
)


if not fuentes_102a_disponibles:

    st.info(
        "10.2.A está en espera. "
        "Las fuentes necesarias serán utilizadas "
        "cuando estén disponibles desde 10.1."
    )
    ...
    if errores_102a:
        ...
        for error_102a in errores_102a:

            st.error(
                f"10.2.A ERROR: {error_102a}"
            )
        ...
        faltantes_102a = [
            campo
            for campo in campos_agrupacion_102a
            if campo not in banco_102a.columns
        ]
        ...
        if faltantes_102a:
            ...
            st.error(
                "10.2.A ERROR: faltan campos necesarios "
                "en el banco: "
                + ", ".join(faltantes_102a)
            )
...
# ============================================================
# FUNCION PARA LEER EL REPOSITORIO PERSISTENTE
# ============================================================

def cargar_repositorio_11a():
    ...
    url = (
        ...
        f"{GITHUB_REPOSITORIO_11A}/"
        f"contents/{ARCHIVO_GITHUB_11A}"
    )
...
# ============================================================
# CARGAR REPOSITORIO PERSISTENTE
#
# ESTA CARGA ES LA QUE GARANTIZA EL CONSECUTIVO.
# ============================================================

df_repositorio_11a = cargar_repositorio_11a()


# ============================================================
# OBTENER EVALUACIONES PERSISTENTES
# ============================================================

evaluaciones_persistentes_11a = pd.DataFrame(
    columns=COLUMNAS_EVALUACIONES_11A
)


if not df_repositorio_11a.empty:

    if "Tipo_Registro" in df_repositorio_11a.columns:

        df_eval_persistentes = (
            df_repositorio_11a[
                df_repositorio_11a[
                    "Tipo_Registro"
                ]
        ...
        df_eval_persistentes = pd.DataFrame()


    if not df_eval_persistentes.empty:

        for columna in COLUMNAS_EVALUACIONES_11A:

            if columna not in df_eval_persistentes.columns:

                df_eval_persistentes[columna] = ""


        evaluaciones_persistentes_11a = (
            df_eval_persistentes[
                COLUMNAS_EVALUACIONES_11A
            ]
            .copy()
...
# ============================================================
# INICIALIZAR EVALUACIONES TEMPORALES
# ============================================================
...
if "evaluaciones_controladas_11a" not in st.session_state:
    ...
    ] = evaluaciones_persistentes_11a.copy()
    ...
    for columna in COLUMNAS_EVALUACIONES_11A:
        ...
        if columna not in df_temporal_11a.columns:

            df_temporal_11a[columna] = ""
    ...
    # --------------------------------------------------------
    # INCORPORAR EVALUACIONES PERSISTENTES QUE NO ESTEN
    # EN EL TEMPORAL
    # --------------------------------------------------------

    if not evaluaciones_persistentes_11a.empty:
        ...
        if not evaluaciones_nuevas.empty:

            df_temporal_11a = pd.concat(
                [
                    df_temporal_11a,
                    evaluaciones_nuevas
                ],
...
# ============================================================
# CONSECUTIVO REAL DE EVALUACION
#
# SE CALCULA CON EL REPOSITORIO PERSISTENTE + TEMPORAL.
# ============================================================

def siguiente_evaluacion_11a(
    df_temporal,
    df_persistente
):
    ...
    if (
        df_persistente is not None
        and not df_persistente.empty
        and "Evaluacion_ID"
        in df_persistente.columns
    ):
    ...
    if (
        df_temporal is not None
        and not df_temporal.empty
        and "Evaluacion_ID"
        in df_temporal.columns
    ):
...
st.markdown("### 1. Crear evaluación")
...
# ============================================================
# 2. SI NO HAY EVALUACIONES
# ============================================================

if df_evaluaciones_11a.empty:

    st.info(
        "Todavía no existen evaluaciones controladas."
    )

else:

    # ========================================================
    # 3. EVALUACIONES CREADAS
    # ========================================================
    ...
    # ========================================================
    # 4. SELECCIONAR EVALUACION
    # ========================================================

    st.markdown(
        "### 3. Trabajar evaluación"
    )
    ...
    if (
        ...
        if nivel == 1:
            ...
            respuesta_correcta_n1 = st.radio(
            ...
            respuestas_correctas = (
                respuesta_correcta_n1
            )


        else:

            respuestas_correctas_n2 = (
                st.multiselect(
                    "Seleccione las dos "
                    "respuestas correctas",
                    [
        ...
        if st.button(
            ...
            if nivel == 2:
                ...
                if len(
                    ...
                    errores.append(
                        "En Nivel 2 debe seleccionar "
                        "exactamente dos respuestas correctas."
                    )
    ...
    # ========================================================
        ...
        st.success(
            ...
            f"{preguntas_creadas} de "
            f"{cantidad_actual} preguntas."
        )


    # ========================================================
    # 12. EVALUACION INACTIVA
    # ========================================================
...
# ============================================================
# IMPORT LOCAL
# NO MODIFICA LOS IMPORTS DEL INICIO DE evaluacion.py
# ============================================================
...
GITHUB_TOKEN_11B = st.secrets["GITHUB_TOKEN"]

GITHUB_USUARIO_11B = "franquiciasauces"

GITHUB_REPOSITORIO_11B = "Asesores"

GITHUB_RAMA_11B = "main"
...
#
# UNA FILA = UNA EVALUACION COMPLETA
# ============================================================
...
# ============================================================
# TOMAR LAS EVALUACIONES CREADAS EN 11.A
# ============================================================
...
# ============================================================
# VERIFICAR QUE EXISTA UNA EVALUACION ACTIVA
# ============================================================

if (
    df_evaluaciones_11b.empty
    or evaluacion_activa_11b is None
):
    ...
    # ========================================================
    # ID DE LA EVALUACION ACTIVA
    # ========================================================

    evaluacion_id_11b = str(
        evaluacion_activa_11b
    )
    ...
    if fila_evaluacion_11b.empty:
        ...
        st.error(
            "11.B no pudo localizar la evaluación "
            "activa generada por 11.A."
        )
        ...
        # ====================================================
        # BUSCAR LAS PREGUNTAS EN EL TEMPORAL DE 11.A
        # ====================================================
        ...
        c3.metric(
            "Preguntas",
            f"{preguntas_creadas_11b}/{cantidad_11b}"
        )
        ...
        if preguntas_creadas_11b < cantidad_11b:

            st.warning(
                f"La evaluación {evaluacion_id_11b} "
                f"todavía no está completa. "
                f"Tiene {preguntas_creadas_11b} de "
            ...
            st.success(
                f"La evaluación {evaluacion_id_11b} "
                "está completa y lista para persistir."
            )
            ...
            if "Pregunta_ID" in df_preguntas_11b.columns:

                df_preguntas_11b["_ORDEN_11B"] = (
                    df_preguntas_11b[
                        "Pregunta_ID"
                    ]
                    .astype(str)
            ...
            # =================================================
            # LISTAS PARA CONSTRUIR UNA SOLA FILA
            # =================================================
            ...
            for indice, (_, pregunta_11b) in enumerate(
                df_preguntas_11b.iterrows(),
                start=1
            ):
                ...
                pregunta_id_11b = str(
                    pregunta_11b.get(
                        "Pregunta_ID",
                        f"{evaluacion_id_11b}_P{indice:03d}"
                    )
                )
                ...
                lista_opcion_1_11b.append(
                    f"{pregunta_id_11b} | "
                    f"{opcion_1_11b}"
                )


                lista_opcion_2_11b.append(
                    f"{pregunta_id_11b} | "
                    f"{opcion_2_11b}"
                )


                lista_opcion_3_11b.append(
                    f"{pregunta_id_11b} | "
                    f"{opcion_3_11b}"
                )


                lista_opcion_4_11b.append(
                    f"{pregunta_id_11b} | "
                    f"{opcion_4_11b}"
                )
            ...
            # =================================================
            # CREAR UNA SOLA FILA PARA LA EVALUACION
            # =================================================

            registro_11b = {
                "Evaluacion_ID": evaluacion_id_11b,
                "Modulo": modulo_11b,
                "Nombre_Evaluacion": nombre_11b,
                "Descripcion": descripcion_11b,
                "Cantidad_Preguntas": cantidad_11b,
                "Preguntas_Creadas": preguntas_creadas_11b,
                "Estado": estado_11b,
                "Pregunta": "\n".join(
                    lista_preguntas_11b
                ),
            ...
            st.markdown(
                "### Evaluación que será persistida"
            )
            ...
            if st.button(
                ...
                url_11b = (
                    f"https://api.github.com/repos/"
                    f"{GITHUB_USUARIO_11B}/"
                    f"{GITHUB_REPOSITORIO_11B}/"
                    f"contents/"
                    f"{ARCHIVO_GITHUB_11B}"
                )


                headers_11b = {
                    "Authorization": (
                        f"Bearer {GITHUB_TOKEN_11B}"
                    ),
                    "Accept": (
                        "application/vnd.github+json"
                    )
                ...
                respuesta_get_11b = requests.get(
                    url_11b,
                    headers=headers_11b,
                    params={
                        "ref": GITHUB_RAMA_11B
                ...
                if respuesta_get_11b.status_code == 404:

                    df_repositorio_11b = pd.DataFrame(
                        columns=COLUMNAS_REPOSITORIO_11B
                    )
                ...
                # =============================================
                ...
                elif respuesta_get_11b.status_code == 200:

                    datos_archivo_11b = (
                        respuesta_get_11b.json()
                    )


                    sha_11b = datos_archivo_11b.get(
                        "sha"
                    )


                    contenido_archivo_11b = (
                        datos_archivo_11b.get(
                            "content",
                            ""
                        )
                    )
                    ...
                    if contenido_archivo_11b:
                        ...
                        df_repositorio_11b = pd.read_csv(
                            StringIO(
                                contenido_decodificado_11b
                            ),
                            dtype=str
                ...
                # =============================================
                    ...
                    st.code(
                        respuesta_get_11b.text
                    )


                    df_repositorio_11b = None

                    sha_11b = None


                # =============================================
                # CONTINUAR SI LA LECTURA FUE CORRECTA
                # =============================================

                if df_repositorio_11b is not None:
                    ...
                    for columna in COLUMNAS_REPOSITORIO_11B:
                        ...
                        if columna not in df_repositorio_11b.columns:
                    ...
                    # =========================================
                    # QUITAR LA VERSION ANTERIOR
                    # DE ESTA MISMA EVALUACION
                    # =========================================
                    ...
                    df_repositorio_11b = (
                        df_repositorio_11b[
                            ...
                            .astype(str)
                            .ne(evaluacion_id_11b)
                        ]
                    )


                    # =========================================
                    # AGREGAR LA EVALUACION ACTUAL
                    # =========================================
                    ...
                    respuesta_put_11b = requests.put(
                        url_11b,
                        headers=headers_11b,
                        json=payload_11b,
                        timeout=30
                    ...
                    if respuesta_put_11b.status_code in [
                        200,
                        201
                    ]:
                        ...
                        st.write(
                            ...
                            f"{preguntas_creadas_11b}"
                        )


                        st.write(
                            "Evaluaciones en el repositorio: "
