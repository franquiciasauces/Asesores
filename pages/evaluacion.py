...
# ============================================================
# 11.A - EVALUACIONES CONTROLADAS
# ============================================================

st.subheader("11.A - Evaluaciones Controladas")

# ------------------------------------------------------------
# INICIALIZAR VARIABLES DE SESION
# ------------------------------------------------------------

if "ec_actual" not in st.session_state:
    st.session_state.ec_actual = None

if "ec_preguntas" not in st.session_state:
    st.session_state.ec_preguntas = []

if "ec_codigo" not in st.session_state:
    st.session_state.ec_codigo = 1

if "ec_finalizadas" not in st.session_state:
    st.session_state.ec_finalizadas = []

if "ec_version_formulario" not in st.session_state:
    st.session_state.ec_version_formulario = 0
...
if st.session_state.ec_actual is None:
    ...
    if st.button(
        ...
        if not nombre_evaluacion.strip():
            ...
            codigo = f"EC_{st.session_state.ec_codigo:05d}"
            st.session_state.ec_codigo += 1

            st.session_state.ec_actual = {
                "Evaluacion_ID": codigo,
                "Nombre_Evaluacion": nombre_evaluacion.strip(),
                "Tema_Evaluacion": tema_evaluacion.strip(),
            ...
            st.session_state.ec_preguntas = []
            st.session_state.ec_version_formulario += 1

            st.rerun()


# ------------------------------------------------------------
# CONSTRUCCION DE LA EVALUACION
# ------------------------------------------------------------

else:

    evaluacion = st.session_state.ec_actual
    ...
    with col1:
        st.write(
            f"**Código:** {evaluacion['Evaluacion_ID']}"
        )
    ...
    st.write(
        f"**Tema:** {evaluacion['Tema_Evaluacion']}"
    )

    st.write(
        f"**Preguntas ingresadas:** "
        f"{len(st.session_state.ec_preguntas)}"
    )

    st.divider()

    if evaluacion["Nivel"] == "Nivel 1":

        st.info(
            "Nivel 1: seleccione exactamente 1 respuesta correcta "
            "y 3 distractores."
        )
        ...
        st.info(
            "Nivel 2: seleccione exactamente 2 respuestas correctas "
            "y 2 distractores."
        )
    ...
    version = st.session_state.ec_version_formulario
    ...
    modulo = st.selectbox(
        ...
        key=f"ec_modulo_{version}"
    )

    tema = st.text_input(
        "Tema",
        value=evaluacion["Tema_Evaluacion"],
        key=f"ec_tema_{version}"
    )

    pregunta = st.text_area(
        "Pregunta",
        key=f"ec_pregunta_{version}",
        height=100
    )
    ...
    respuesta_1 = st.text_input(
        "Respuesta 1",
        key=f"ec_respuesta_1_{version}"
    )

    respuesta_2 = st.text_input(
        "Respuesta 2",
        key=f"ec_respuesta_2_{version}"
    )

    respuesta_3 = st.text_input(
        "Respuesta 3",
        key=f"ec_respuesta_3_{version}"
    )

    respuesta_4 = st.text_input(
        "Respuesta 4",
        key=f"ec_respuesta_4_{version}"
    )

    opciones_correctas = st.multiselect(
    ...
    if st.button(
        ...
        if not pregunta.strip():
        ...
        elif any(not respuesta for respuesta in respuestas):

            st.error("Debe diligenciar las cuatro respuestas.")

        elif len(opciones_correctas) != cantidad_correctas:

            st.error(
                f"Para {evaluacion['Nivel']} debe seleccionar "
                f"exactamente {cantidad_correctas} respuesta(s) correcta(s)."
            )
            ...
            for opcion in opciones_correctas:

                if opcion == "Respuesta 1":
                    correctas.append(1)

                elif opcion == "Respuesta 2":
                    correctas.append(2)

                elif opcion == "Respuesta 3":
                    correctas.append(3)

                elif opcion == "Respuesta 4":
                    correctas.append(4)
            ...
            nueva_pregunta = {
                ...
                "Respuesta_Correcta_1": correctas[0],
            ...
            st.session_state.ec_preguntas.append(
                nueva_pregunta
            )
    ...
    # --------------------------------------------------------
    # MOSTRAR PREGUNTAS INGRESADAS
    # --------------------------------------------------------

    if st.session_state.ec_preguntas:

        st.divider()

        st.markdown(
            f"### Preguntas de la evaluación "
            f"({len(st.session_state.ec_preguntas)})"
        )

        for pregunta_guardada in st.session_state.ec_preguntas:

            with st.expander(
                f"{pregunta_guardada['Pregunta_ID']} - "
                f"{pregunta_guardada['Tema']}"
            ):

                st.write(
                    f"**Módulo:** "
                    f"{pregunta_guardada['Modulo']}"
                )

                st.write(
                    f"**Nivel:** "
                    f"{pregunta_guardada['Nivel']}"
                )

                st.write(
                    f"**Pregunta:** "
                    f"{pregunta_guardada['Pregunta']}"
                )

                st.write(
                    f"**1.** "
                    f"{pregunta_guardada['Respuesta_1']}"
                )

                st.write(
                    f"**2.** "
                    f"{pregunta_guardada['Respuesta_2']}"
                )

                st.write(
                    f"**3.** "
                    f"{pregunta_guardada['Respuesta_3']}"
                )

                st.write(
                    f"**4.** "
                    f"{pregunta_guardada['Respuesta_4']}"
                )

                correctas_mostrar = [
                    str(pregunta_guardada["Respuesta_Correcta_1"])
                ]

                if pregunta_guardada["Respuesta_Correcta_2"] is not None:
                    correctas_mostrar.append(
                        str(pregunta_guardada["Respuesta_Correcta_2"])
                    )

                st.write(
                    "**Respuesta(s) correcta(s):** "
                    + ", ".join(correctas_mostrar)
                )
    ...
    if st.session_state.ec_preguntas:
        ...
        if st.button(
            ...
            type="primary",
            key="ec_finalizar_evaluacion"
        ):

            evaluacion_finalizada = evaluacion.copy()

            evaluacion_finalizada["Numero_Preguntas"] = len(
                st.session_state.ec_preguntas
            )

            evaluacion_finalizada["Preguntas"] = (
                st.session_state.ec_preguntas.copy()
            )

            st.session_state.ec_finalizadas.append(
        ...
        st.warning(
            "Debe agregar al menos una pregunta antes de "
            "finalizar la evaluación."
        )


# ============================================================
# EVALUACIONES FINALIZADAS EN LA SESION
# ============================================================

if st.session_state.ec_finalizadas:

    st.divider()

    st.markdown("### Evaluaciones finalizadas")

    for evaluacion_finalizada in st.session_state.ec_finalizadas:

        st.write(
            f"**{evaluacion_finalizada['Evaluacion_ID']}** - "
            f"{evaluacion_finalizada['Nombre_Evaluacion']} - "
            f"{evaluacion_finalizada['Tema_Evaluacion']} - "
            f"{evaluacion_finalizada['Nivel']} - "
            f"{evaluacion_finalizada['Estado']} - "
            f"{evaluacion_finalizada['Numero_Preguntas']} preguntas"
        )
