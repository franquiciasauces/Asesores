...
if "ec_numero" not in st.session_state:
    st.session_state.ec_numero = 1
...
if st.session_state.ec_actual is None:
    ...
    nombre = st.text_input("Nombre de la evaluación")
    tema = st.text_input("Tema de la evaluación")
    nivel = st.selectbox("Nivel", ["Nivel 1", "Nivel 2"])

    if st.button("Crear evaluación", key="ec_crear"):

        if not nombre.strip() or not tema.strip():
            st.error("Debe diligenciar el nombre y el tema.")

        else:

            st.session_state.ec_actual = {
                "Evaluacion_ID": f"EC_{st.session_state.ec_numero:05d}",
                "Nombre_Evaluacion": nombre.strip(),
                "Tema_Evaluacion": tema.strip(),
                "Nivel": nivel,
                "Estado": "Activa",
...
if st.session_state.ec_actual is not None:
    ...
    st.write(
        f"**Preguntas:** {len(st.session_state.ec_preguntas)}"
    )
    ...
    if st.button(
        ...
        key="ec_agregar_pregunta"
    ):

        if not pregunta.strip():
            st.error("Debe escribir la pregunta.")
            ...
            numero = len(st.session_state.ec_preguntas) + 1

            st.session_state.ec_preguntas.append({
                "Evaluacion_ID": evaluacion["Evaluacion_ID"],
                "Pregunta_ID": (
                    f"{evaluacion['Evaluacion_ID']}_P{numero:03d}"
                ),
                "Modulo": modulo,
...
# ------------------------------------------------------------
# MOSTRAR PREGUNTAS Y FINALIZAR
# ------------------------------------------------------------

if (
    st.session_state.ec_actual is not None
    and st.session_state.ec_preguntas
):
    ...
    st.dataframe(
        pd.DataFrame(st.session_state.ec_preguntas),
        use_container_width=True,
        hide_index=True
    )
    ...
    if st.button(
        ...
        st.session_state.ec_actual["Numero_Preguntas"] = len(
            st.session_state.ec_preguntas
        )

        st.session_state.ec_actual["Preguntas"] = (
        ...
        st.success(
            ...
            "La sincronización se realizará en 11.B."
        )
