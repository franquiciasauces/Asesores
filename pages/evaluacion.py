...
# El Banco General NO SE MODIFICA.
#
# 10.2 hace un CRUCE de información:
#
#       BANCO GENERAL
#              +
#       PREGUNTAS_EVALUACIONES
#
# El cruce se realiza por Pregunta_ID.
#
# SOLO DESCUENTAN UNA PREGUNTA:
#
#       APROBADO
#       RECHAZADO
#
...
# IMPORTANTE:
#
# - Que un Pregunta_ID aparezca en PREGUNTAS_EVALUACIONES
#   NO significa automáticamente que esté consumido.
#
# - Una pregunta se descuenta UNA SOLA VEZ.
#
# - Los distractores NO se descuentan como preguntas
#   adicionales.
#
# - EVALUACIONES.csv solamente informa sobre las
#   evaluaciones registradas. NO se utiliza para descontar
#   preguntas.
#
# - El análisis se realiza también por:
#       Modulo
#       Tipo_Relacion
#       Nivel
#
...
# NORMALIZAR ESTADO
#
# Se aceptan pequeñas diferencias de mayúsculas,
# espacios y tildes.
# ============================================================

def normalizar_estado_102(valor):

    texto_102 = normalizar_texto_102(valor).upper()
...
# ============================================================
# OBTENER DATAFRAME DE PREGUNTAS_EVALUACIONES
#
# 10.1 ya lo cargó.
#
# NO SE VUELVE A LEER GITHUB.
# ============================================================

def obtener_preguntas_evaluaciones_102():

    posibles_claves_102 = [
        "df_preguntas_evaluaciones_101",
        "df_preguntas_evaluaciones_102",
        "preguntas_evaluaciones",
        "df_preguntas_evaluaciones"
    ]

    for clave_102 in posibles_claves_102:

        valor_102 = st.session_state.get(
            clave_102
        )

        if isinstance(valor_102, pd.DataFrame):

            return valor_102.copy()
...
# ============================================================
# ENCONTRAR COLUMNA DE ESTADO REAL
#
# PREGUNTAS_EVALUACIONES puede haber sido generado con
...
# ============================================================
# CREAR ESTADO DEFINITIVO POR Pregunta_ID
#
# REGLA:
#
# APROBADO  -> DESCARTA
# RECHAZADO  -> DESCARTA
# NO APLICA AUN -> DISPONIBLE
#
# ============================================================

def construir_estado_validacion_102(
    ...
    #
    # Si un mismo ID aparece varias veces:
    #
    # APROBADO / RECHAZADO
    ...
    for _, fila_102 in (
        ...
        if estado_102 in [
            ...
            estado_102 = "NO APLICA AUN"

        # ----------------------------------------------------
        # SOLO LOS TRES ESTADOS VALIDOS INTERESAN
        # ----------------------------------------------------

        if estado_102 not in [
            "APROBADO",
            "RECHAZADO",
            "NO APLICA AUN"
        ]:
        ...
        if estado_anterior_102 is None:
...
def analizar_banco_102(
    ...
    #
    # AQUÍ SE PRODUCE EL CRUCE REAL.
    # ========================================================

    estados_validacion_102 = (
        construir_estado_validacion_102(
            df_preguntas_eval_102
        )
    )

    # ========================================================
    # ASIGNAR ESTADO DE VALIDACIÓN AL BANCO
    # ========================================================

    banco_102[
        "_Estado_Validacion_102"
    ] = (
        banco_102[
            "Pregunta_ID"
        ]
    ...
    # ========================================================
    # CLASIFICAR EL RESULTADO DEL CRUCE
    # ========================================================
    ...
    # IMPORTANTE:
    #
    # LA DISPONIBILIDAD REAL NO PUEDE UTILIZAR PREGUNTAS
    # QUE EL BANCO GENERAL YA MARCA COMO RECHAZADAS.
    #
    # EL BANCO GENERAL APROBADO + NO DESCONTADO = DISPONIBLE
    #
    # El estado NO APLICA AUN queda disponible.
    # ========================================================
    ...
    ids_aprobados_validacion_102 = set(
        banco_102.loc[
            banco_102[
                "_Estado_Validacion_102"
            ] == "APROBADO",
    ...
    ids_rechazados_validacion_102 = set(
        banco_102.loc[
            banco_102[
                "_Estado_Validacion_102"
            ] == "RECHAZADO",
    ...
    ids_descontados_102 = (
        ids_aprobados_validacion_102
        |
        ids_rechazados_validacion_102
    )
    ...
    preguntas_con_estado_validacion_102 = len(
        banco_102[
            banco_102[
                "_Estado_Validacion_102"
            ] != ""
    ...
    preguntas_descontadas_102 = len(
        ids_descontados_102
    )

    preguntas_aprobadas_validacion_102 = len(
        ids_aprobados_validacion_102
    )

    preguntas_rechazadas_validacion_102 = len(
        ids_rechazados_validacion_102
    )

    preguntas_no_aplica_102 = len(
        ids_no_aplica_102
    )
    ...
    # ========================================================
    # FUNCIÓN INTERNA PARA GENERAR RESUMEN POR GRUPO
    # ========================================================
    ...
    # ========================================================
    # PREGUNTAS DESCONTADAS POR GRUPO
    # ========================================================

    descontadas_grupo_102 = (
        banco_102[
            banco_102[
                "_Descuenta_102"
            ]
    ...
    resumen_102 = (
        ...
        .merge(
            descontadas_grupo_102,
            on=columnas_grupo_102,
            how="left"
        )
    )

    # ========================================================
    # APROBADAS EN VALIDACIÓN POR GRUPO
    # ========================================================

    aprobadas_val_grupo_102 = (
        banco_102[
            banco_102[
                "_Estado_Validacion_102"
            ] == "APROBADO"
    ...
    # ========================================================
    # RECHAZADAS EN VALIDACIÓN POR GRUPO
    # ========================================================

    rechazadas_val_grupo_102 = (
        banco_102[
            banco_102[
                "_Estado_Validacion_102"
            ] == "RECHAZADO"
    ...
    columnas_numericas_102 = [
        "Preguntas_Disponibles",
        "Preguntas_Descontadas",
        "Validadas_Aprobadas",
        "Validadas_Rechazadas",
        "No_Aplica_Aun",
        "Rechazadas_Banco"
    ]

    for columna_102 in columnas_numericas_102:

        if columna_102 not in resumen_102.columns:
    ...
    # ========================================================
    # DISPONIBLES CON UNA REGLA CLARA
    #
    # NO SE DECIDE AQUÍ CUÁNTAS EVALUACIONES CREAR.
    #
    # SOLO SE INFORMA CUÁNTAS PREGUNTAS HAY DISPONIBLES.
    #
    # ========================================================

    resumen_102[
        "Cola_Si_Se_Trabaja_En_10"
    ] = (
        resumen_102[
            "Preguntas_Disponibles"
        ]
    ...
    # ========================================================
    # MÁXIMO TEÓRICO DE GRUPOS DE 10
    #
    # ES SOLO INFORMACIÓN.
    # NO ES UNA DECISIÓN DE GENERACIÓN.
    # ========================================================

    resumen_102[
        "Grupos_Completos_De_10"
    ] = (
        resumen_102[
            "Preguntas_Disponibles"
        ]
    ...
    return {
        ...
        "pendientes_banco":
            pendientes_banco_102,

        "preguntas_con_estado_validacion":
            preguntas_con_estado_validacion_102,

        "preguntas_descontadas":
            preguntas_descontadas_102,

        "validadas_aprobadas":
            preguntas_aprobadas_validacion_102,

        "validadas_rechazadas":
            preguntas_rechazadas_validacion_102,

        "no_aplica_aun":
            preguntas_no_aplica_102,
...
st.info(
    "10.2 cruza el Banco General con el estado real de "
    "cada Pregunta_ID registrado en la validación. "
    "El Banco General no se modifica."
)


# ============================================================
# OBTENER FUENTES YA CARGADAS POR 10.1
# ============================================================
...
df_preguntas_eval_102 = (
    obtener_preguntas_evaluaciones_102()
)

df_evaluaciones_102 = (
    obtener_evaluaciones_102()
)
...
if df_banco_102.empty:
    ...
    if st.button(
        "ACTUALIZAR ANÁLISIS 10.2",
        key="actualizar_analisis_real_102"
    ):

        # Volver a tomar los DataFrames de 10.1.
        # NO volver a leer GitHub.

        df_banco_actual_102 = st.session_state.get(
            "df_banco_101",
            pd.DataFrame()
        )
...
if resultado_analisis_102:
    ...
    total_102 = resultado_analisis_102[
        "total"
    ]

    aprobadas_banco_102 = resultado_analisis_102[
        "aprobadas_banco"
    ]

    rechazadas_banco_102 = resultado_analisis_102[
        "rechazadas_banco"
    ]

    pendientes_banco_102 = resultado_analisis_102[
        "pendientes_banco"
    ]
    ...
    validadas_aprobadas_102 = (
        resultado_analisis_102[
            "validadas_aprobadas"
        ]
    )

    validadas_rechazadas_102 = (
        resultado_analisis_102[
            "validadas_rechazadas"
        ]
    )
    ...
    st.markdown(
        "### Fuentes utilizadas por 10.2"
    )

    colf1_102, colf2_102, colf3_102 = st.columns(3)

    with colf1_102:

        st.success(
            "BANCO GENERAL\n"
            f"{total_102:,} preguntas"
        )
    ...
    with colf2_102:
        ...
        st.success(
            "PREGUNTAS_EVALUACIONES\n"
            f"{len(df_preguntas_eval_102):,} registros"
        )
    ...
    with colf3_102:
        ...
        st.success(
            "EVALUACIONES\n"
            f"{evaluaciones_registradas_102:,} registradas"
        )
    ...
    st.markdown(
        "### Estado general del Banco General"
    )

    c1_102, c2_102, c3_102, c4_102 = st.columns(4)
    ...
    st.markdown(
        "### Cruce real de consumo y validación"
    )

    st.caption(
        "Una pregunta solo se descuenta una vez cuando su "
        "Pregunta_ID tiene estado APROBADO o RECHAZADO en "
        "la validación. NO APLICA AÚN permanece disponible."
    )

    c5_102, c6_102, c7_102, c8_102 = st.columns(4)

    with c5_102:

        st.metric(
            "Registros con estado de validación",
            f"{preguntas_con_estado_validacion_102:,}"
        )

    with c6_102:

        st.metric(
            "Descontadas del banco",
            f"{preguntas_descontadas_102:,}"
        )

    with c7_102:

        st.metric(
            "APROBADO en validación",
            f"{validadas_aprobadas_102:,}"
        )
    ...
    with c8_102:
        ...
        st.metric(
            ...
            f"{validadas_rechazadas_102:,}"
        )


    # ========================================================
    # NO APLICA AÚN / DISPONIBILIDAD
    # ========================================================

    c9_102, c10_102, c11_102 = st.columns(3)
    ...
    with c11_102:
        ...
        st.metric(
            "Evaluaciones registradas",
            f"{evaluaciones_registradas_102:,}"
        )
    ...
    st.markdown(
        "### Cómo está haciendo el descuento"
    )

    st.write(
        f"El Banco General contiene {total_102:,} preguntas. "
        f"De ellas, {preguntas_descontadas_102:,} tienen un "
        "estado definitivo de validación APROBADO o RECHAZADO "
        "y por eso dejan de estar disponibles."
    )

    st.write(
        f"Hay {no_aplica_aun_102:,} preguntas con estado "
        "NO APLICA AÚN. Estas NO se descuentan y permanecen "
        "disponibles."
    )

    st.write(
        "Los distractores u otras apariciones de una pregunta "
        "en una evaluación no generan descuentos adicionales. "
        "El descuento se hace por Pregunta_ID único."
    )
    ...
    st.markdown(
        "### Control de Pregunta_ID"
    )

    if duplicados_102 > 0:

        st.warning(
            f"Se detectaron {duplicados_102:,} registros "
            "duplicados de Pregunta_ID en el Banco General. "
    ...
    st.info(
        "10.2 NO genera evaluaciones y NO decide qué preguntas "
        "usar. Solo determina qué Pregunta_ID siguen disponibles. "
        "10.3 deberá utilizar esta disponibilidad para generar "
        "nuevas evaluaciones y excluir únicamente los IDs cuyo "
        "estado de validación sea APROBADO o RECHAZADO."
    )
============================================================
# FIN 10.2
# ============================================================
...
# ============================================================
# FITOASISTE
# 10.3 - GENERADOR / PREPARADOR DE EVALUACIONES
# ============================================================
#
# OBJETIVO:
#
...
#       - EVALUACIONES.csv
#
#   La evaluación queda únicamente en session_state.
#
# ============================================================
...
GITHUB_USUARIO_103 = "franquiciasauces"

GITHUB_REPOSITORIO_103 = "Asesores"

GITHUB_RAMA_103 = "main"
...
ARCHIVO_PREGUNTAS_EVALUACIONES_103 = (
    "page/PREGUNTAS_EVALUACIONES.csv"
)

ARCHIVO_EVALUACIONES_103 = (
    "page/EVALUACIONES.csv"
)
...
try:
    ...
    GITHUB_TOKEN_103 = st.secrets["GITHUB_TOKEN"]
...
def cargar_csv_github_103(
    ruta_archivo_103
):

    if not GITHUB_TOKEN_103:
    ...
    try:
        ...
        solicitud_103 = urllib.request.Request(
        ...
        if "content" not in datos_103:

            return None


        contenido_103 = base64.b64decode(
        ...
        return df_103.fillna("")


    except urllib.error.HTTPError as error_103:

        # ----------------------------------------------------
        # 404 = archivo todavía no existe.
        #
        # Esto NO es un error fatal para 10.3.
        # ----------------------------------------------------

        if error_103.code == 404:
...
# ============================================================
# OBTENER PREGUNTA_ID REALMENTE USADOS
#
# REGLA:
#
# SOLAMENTE se considera usada una pregunta cuando su estado
# sea explícitamente uno de:
#
#     USADA
#     USADO
#     UTILIZADA
...
#
# Esto es fundamental.
# Preparar una evaluación NO significa consumir preguntas.
# ============================================================

def obtener_preguntas_usadas_103(
    df_preguntas_evaluaciones_103
):

    if (
        df_preguntas_evaluaciones_103 is None
        or
        df_preguntas_evaluaciones_103.empty
    ):
    ...
    if "Pregunta_ID" not in (
        df_preguntas_evaluaciones_103.columns
    ):
    ...
    columna_estado_103 = None


    posibles_estados_103 = [
    ...
    for columna_103 in posibles_estados_103:
    ...
    #
    # NO asumir que las preguntas están usadas.
    # --------------------------------------------------------

    if columna_estado_103 is None:

        return set()


    # --------------------------------------------------------
    # ÚNICOS estados que bloquean reutilización
    # --------------------------------------------------------
...
# ============================================================
# CREAR IDENTIFICADOR TEMPORAL DE EVALUACIÓN
#
# IMPORTANTE:
#
# Este ID NO se escribe en ningún archivo.
#
# Sirve únicamente para que 10.4 pueda identificar el bloque
# que acaba de preparar 10.3.
# ============================================================

def crear_evaluacion_id_temporal_103():
...
# ============================================================
# CARGAR BANCO DESDE SESSION STATE
#
# 10.1 es responsable de cargar el banco.
#
# 10.3 NO vuelve a leer BANCO_PREGUNTAS_GENERALES.xlsx.
# ============================================================

df_banco_103 = st.session_state.get(
...
if df_banco_103.empty:
    ...
    #
    # NO se cambian nombres del banco.
    # Solamente se eliminan espacios accidentales.
    # ========================================================
    ...
    df_banco_103.columns = [

        str(columna_103).strip()

        for columna_103
        in df_banco_103.columns
    ...
    faltantes_103 = [
        ...
        for columna_103
        in COLUMNAS_BANCO_103

        if columna_103
        not in df_banco_103.columns
    ...
    if faltantes_103:
        ...
        # ====================================================
        # NORMALIZAR Pregunta_ID PARA COMPARACIÓN
        #
        # NO modifica el ID original del banco.
        # Se utiliza una columna interna.
        # ====================================================

        df_banco_103[
            "_Pregunta_ID_COMPARACION_103"
        ] = (
        ...
        # SOLO LECTURA.
        #
        # No se utiliza para marcar preguntas como usadas.
        # ====================================================
        ...
        # ====================================================
        # DETERMINAR Pregunta_ID REALMENTE USADOS
        # ====================================================

        preguntas_usadas_103 = (

            obtener_preguntas_usadas_103(
        ...
        # ====================================================
        # GUARDAR CONTROL EN SESSION STATE
        # ====================================================
        ...
        st.markdown(
            "## 10.3 - Generador de evaluaciones"
        )


        st.info(

            "10.3 solamente prepara una evaluación "
            "temporal. No guarda cambios en GitHub, "
            "no modifica el banco y no marca preguntas "
            "como usadas."
        ...
        no_usadas_banco_103 = (
            ...
            len(ids_banco_103)
            -
            len(usados_que_estan_en_banco_103)
        ...
        # ====================================================
        # ELIMINAR PREGUNTAS REALMENTE USADAS
        # ====================================================
        ...
        # ====================================================
        # ELIMINAR DUPLICADOS DE Pregunta_ID
        #
        # No se permite que una misma pregunta aparezca dos
        # veces en la misma evaluación.
        #
        # Se conserva la primera aparición exactamente como
        # está en el banco.
        # ====================================================
        ...
        if not modulos_103:
            ...
            st.warning(
                ...
                "No existen preguntas APROBADAS y "
                "NO USADAS disponibles para generar "
                "una evaluación."
            ...
            if not relaciones_103:
                ...
                relacion_seleccionada_103 = st.selectbox(

                    "Tipo_Relacion",

                    relaciones_103,

                    key="generador_relacion_103"
                ...
                if not niveles_103:
                    ...
                    nivel_seleccionado_103 = st.selectbox(

                        "Nivel",

                        niveles_103,

                        key="generador_nivel_103"
                    ...
                    st.info(
                        ...
                        f"{modulo_seleccionado_103} / "

                        f"{relacion_seleccionada_103} / "

                        f"{nivel_seleccionado_103} "

                        f"hay {disponibles_103} "

                        "preguntas APROBADAS y NO USADAS."
                    ...
                    if disponibles_103 >= 10:
                        ...
                        cantidad_103 = st.number_input(

                            "Cantidad de preguntas",

                            min_value=1,

                            max_value=disponibles_103,

                            value=10,

                            step=1,

                            key="generador_cantidad_103"

                        )


                        confirmar_menor_103 = True


                    elif disponibles_103 > 0:

                        st.warning(

                            f"Solo existen {disponibles_103} "
                            "preguntas disponibles para "
                            "esta combinación. "

                            "La evaluación será menor de 10."
                        ...
                        cantidad_103 = st.number_input(
                            ...
                            value=disponibles_103,

                            step=1,

                            key="generador_cantidad_menor_103"

                        )


                        confirmar_menor_103 = st.checkbox(

                            "Confirmo generar una evaluación "
                            "menor de 10 preguntas.",

                            key=
                            "confirmar_evaluacion_menor_103"
                    ...
                    if st.button(
                        ...
                        key=
                        "generar_evaluacion_103",

                        disabled=
                        not puede_generar_103
                        ...
                        # SELECCIÓN
                        #
                        # SE CONSERVA EL ORDEN DEL BANCO.
                        #
                        ...
                        if (
                            ...
                            st.error(
                                ...
                                "10.3 ERROR: no fue posible "
                                "preparar la cantidad solicitada. "
                                "La evaluación NO fue generada."
                            ...
                            if (
                                ...
                                st.error(
                                    ...
                                    "10.3 ERROR: una de las "
                                    "preguntas seleccionadas "
                                    "tiene Pregunta_ID vacío. "
                                    "La evaluación NO fue generada."
                            ...
                            # =================================
                            ...
                            elif ids_103.duplicated().any():

                                st.error(

                                    "10.3 ERROR: el banco contiene "
                                    "Pregunta_ID duplicados entre "
                                ...
                                if ids_usados_otra_vez_103:

                                    st.error(

                                        "10.3 ERROR: se detectó "
                                        "una pregunta que figura "
                                        "como USADA. "
                                        "La evaluación NO fue generada."
                                    ...
                                    evaluacion_id_103 = (
                                        ...
                                        crear_evaluacion_id_temporal_103()
                                    ...
                                    st.warning(
                                        ...
                                        "IMPORTANTE: esta evaluación "
                                        "todavía NO está persistida. "
                                        "Las preguntas NO han sido "
                                        "marcadas como usadas. "
                                        "10.4 debe validar esta "
                                        "evaluación antes de realizar "
                                        "la persistencia definitiva."

                                    )


                    # =========================================
                    # MOSTRAR EVALUACIÓN TEMPORAL
                    # =========================================
                    ...
                    if st.session_state.get(
                        ...
                        if (
                            ...
                            not
                            df_evaluacion_mostrar_103.empty
                            ...
                            st.caption(
                                ...
                                "Los Pregunta_ID son exactamente "
                                "los del banco original. "
                                "No se genera ningún nuevo "
                                "Pregunta_ID."
                            ...
                            st.info(
                                ...
                                "Esta evaluación está en memoria "
                                "de la sesión y queda disponible "
                                "para 10.4. Todavía no existe "
                                "persistencia."
...
def leer_csv_github_105(
    ...
    if not GITHUB_TOKEN_105:
        ...
        st.error(
            "10.5 ERROR: no está configurado "
            "GITHUB_TOKEN."
        )
    ...
    except urllib.error.HTTPError as error_105:
