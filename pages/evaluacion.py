...
def cargar_banco_101():
    ...
    # Solo normalización de identificación.
    # NO modifica estados.
    for columna in [
        "Pregunta_ID",
        "Modulo",
...
if st.button(
    ...
    # --------------------------------------------------------
    # GUARDAR LAS 3 FUENTES
    # --------------------------------------------------------

    st.session_state["df_banco_101"] = df_banco.copy()
    st.session_state["df_preguntas_evaluaciones_101"] = (
        df_preguntas.copy()
    )
    st.session_state["df_evaluaciones_101"] = (
        df_evaluaciones.copy()
    )

    st.session_state["sha_banco_general_101"] = sha_banco
    st.session_state["sha_preguntas_evaluaciones_101"] = (
        sha_preguntas
    )
    st.session_state["sha_evaluaciones_101"] = (
        sha_evaluaciones
    )

    st.session_state["banco_101_cargado"] = True
    st.session_state["registro_preguntas_101_existe"] = (
        existe_preguntas
    )
    st.session_state["registro_evaluaciones_101_existe"] = (
        existe_evaluaciones
    )

    st.success(
        "10.1: las tres fuentes fueron cargadas."
    )
...
if st.session_state.get(
    ...
    st.info(
        "10.1 solamente carga las tres fuentes. "
