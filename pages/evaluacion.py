...
def obtener_relaciones_consumidas_78():
...
GITHUB_USUARIO_78 = "franquiciasauces"
GITHUB_REPOSITORIO_78 = "Asesores"
GITHUB_RAMA_78 = "main"
...
URL_GITHUB_78 = (
    "https://api.github.com/repos/"
    f"{GITHUB_USUARIO_78}/"
    f"{GITHUB_REPOSITORIO_78}/contents/"
    f"{GITHUB_ARCHIVO_78}"
)


def sincronizar_banco_78():
    ...
    try:
        ...
        if "Pregunta_ID" not in df_banco.columns:
            ...
            st.error(
                "7.8 ERROR: el banco no contiene "
        ...
        for _, pregunta in df_nuevas.iterrows():
            ...
            if pregunta_id in ids_existentes:
        ...
        # ----------------------------------------------------
        # ACTUALIZAR PREGUNTAS YA EXISTENTES
        # ----------------------------------------------------

        for pregunta in filas_actualizadas:

            pregunta_id = (
                pregunta["Pregunta_ID"]
            )

            indice = df_banco.index[
                df_banco["Pregunta_ID"]
                == pregunta_id
            ]
        ...
        if filas_nuevas:
            ...
            df_nuevas_agregar = pd.DataFrame(
                filas_nuevas
            )
        ...
        st.info(
            f"Preguntas aprobadas actualizadas: "
            f"{actualizadas_78:,}"
        )
...
if preguntas_78:
    ...
    if aprobadas_78 > 0:
        ...
        if pendientes_78 > 0:
            ...
            st.warning(
                f"Hay {pendientes_78} preguntas "
                "pendientes. Solo se sincronizarán "
                "las aprobadas."
            )

        if st.button(
            "SINCRONIZAR 7.8 CON BANCO DE PREGUNTAS",
            key="sincronizar_banco_78"
        ):
...
def generar_relacion_79(
    ...
    for _, verdadera in candidatos.iterrows():
        ...
        correcta = str(
            verdadera["Precaución / Contraindicación"]
        ).strip()

        if not producto or not correcta:
        ...
        falsas = falsas[
            falsas["Precaución / Contraindicación"]
            .fillna("")
            .astype(str)
            .str.strip()
            != ""
        ...
        for _, falsa in falsas.iterrows():
            ...
            if opcion.lower() == correcta.lower():

                continue

            if any(
                opcion.lower() == existente.lower()
                for existente in opciones
            ):
...
def generar_preguntas_79(
    ...
    while len(preguntas) < cantidad:
        ...
        pregunta = construir_pregunta_79(
            resultado
        )
...
# ============================================================
# INTERFAZ DEL GENERADOR 7.9
# ============================================================

if "df_disponible_79" in st.session_state:

    st.markdown(
        "### 7.9 - Generador Producto - Restricción"
    )
    ...
    cantidad_79 = st.number_input(
        ...
        value=10,
        step=1,
        key="cantidad_generar_79"
    )

    st.info(
        "Nivel 1: identifica la precaución o "
        "contraindicación correspondiente al producto."
    )

    if st.button(
        "GENERAR PREGUNTAS 7.9",
        key="generar_preguntas_79"
    ):

        nuevas_79 = generar_preguntas_79(
            cantidad_79
        )

        st.session_state[
            "preguntas_generadas_79"
        ] = nuevas_79
...
preguntas_79 = st.session_state.get(
    "preguntas_generadas_79",
    []
)
...
if preguntas_79:
    ...
    st.markdown(
        "### Preguntas generadas 7.9"
    )

    for pregunta in preguntas_79:
