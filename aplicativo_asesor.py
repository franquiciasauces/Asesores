...
# ============================================================
    ...
    if (
        ...
        def separar_operador_principal(
            expresion,
            operador
        ):
            ...
            while posicion < len(texto):
                ...
                # -------------------------------------------
                # BUSCAR OPERADOR AL NIVEL PRINCIPAL
                # -------------------------------------------
                ...
                if nivel == 0:
                    ...
                    if (
                        ...
                        # OPERADORES LÓGICOS
                        #
                        # Deben estar delimitados como
                        # palabras, pero NO se exige que
                        # el operador recibido tenga espacios
                        # alrededor.
                        # -----------------------------------
            ...
            # -----------------------------------------------
            # SI ENCONTRÓ AL MENOS UN OPERADOR
            # -----------------------------------------------
...
if (
    ...
    # ========================================================
    # IDENTIFICAR EVALUACIÓN SELECCIONADA
    # ========================================================
    ...
    if not df_repositorio_ejecucion.empty:
        ...
        columnas_faltantes = [
            columna
            for columna in columnas_necesarias
            if columna not in df_repositorio_ejecucion.columns
        ]

        if columnas_faltantes:

            st.error(
                "El repositorio no tiene la estructura "
                "esperada para ejecutar la evaluación."
            )
...
if (
    ...
    if st.button(
        ...
        if (
            ...
            # =================================================
            # INDICAR QUE ESTE RESULTADO NUEVO DEBE PERSISTIRSE
            #
            # ESTA BANDERA DISPARA AUTOMÁTICAMENTE
            # LA PARTE 4.
            # =================================================
...
if (
    ...
    if not evaluacion_id_seleccionada:
        ...
        # ====================================================
        # 5. IDENTIFICAR EL RESULTADO TEMPORAL
        # ====================================================
        ...
        if resultado_temporal is None:
            ...
            st.error(
                ...
                "pero no se encontró el resultado temporal "
            ...
            # =================================================
            # 8. SOLO PROCESAR SI TODAVÍA NO SE HA PROCESADO
            # =================================================
            ...
            if not st.session_state.get(
                ...
                # =============================================
                # 9. OBTENER INFORMACIÓN DE LA EVALUACIÓN
                # =============================================
                ...
                #
                # USUARIO + EVALUACIÓN + FECHA + HORA
                #
                # Se incluyen microsegundos para reforzar
                # la unicidad del registro.
                # =============================================
                ...
                try:
                    ...
                    # =========================================
                    # 17. VERIFICAR RESPUESTA DE GITHUB
                    # =========================================
                    ...
                    if (
                        ...
                        # =====================================
                        # 20. ASEGURAR TODAS LAS COLUMNAS
                        # =====================================
                        ...
                        # =====================================
                        # 25. SI YA EXISTE LA NOTA
                        # =====================================

                        if ya_tiene_nota:

                            # ---------------------------------
                            # NO MODIFICAR EL REGISTRO EXISTENTE
                            # ---------------------------------
                            ...
                            # ---------------------------------
                            # TERMINAR LA SOLICITUD DE GUARDADO
                            # ---------------------------------
                        ...
                        # =====================================
                            ...
                            # =================================
                            # 27. DATAFRAME DEL NUEVO REGISTRO
                            # =================================
                            ...
                            # =================================
                            # 32. ACTUALIZAR ARCHIVO EN GITHUB
                            # =================================
...
if (
    ...
    # ========================================================
    # 1. ARCHIVO MADRE DE EVALUACIONES GENERALES
    # ========================================================

    ARCHIVO_EVALUACIONES_GENERALES = (
        BASE_DIR
        / "page"
        / "EVALUACIONES.csv"
    )
    ...
    if not ARCHIVO_EVALUACIONES_GENERALES.exists():
        ...
        # ====================================================
        # 3. CARGAR ARCHIVO MADRE
        # ====================================================

        try:

            df_evaluaciones_generales = pd.read_csv(
                ARCHIVO_EVALUACIONES_GENERALES,
                dtype=str,
                keep_default_na=False
            )
            ...
            df_evaluaciones_generales = (
                df_evaluaciones_generales.fillna("")
            )
        ...
        if not df_evaluaciones_generales.empty:
            ...
            if columnas_faltantes:
                ...
                if df_evaluaciones_disponibles.empty:
                    ...
                    st.info(
                        "No existen evaluaciones generales "
...
if (
    ...
    if (
        ...
        # ====================================================
        # 3. SI YA FUE FINALIZADA, NO MOSTRAR LAS PREGUNTAS
        # ====================================================
        ...
        if (
            ...
            # La Parte 3 se encargará de mostrar
            # el resultado y la retroalimentación.
            pass

        else:

            # =================================================
            # 4. UBICAR CARPETA PAGE
            # =================================================
                ...
                if archivo_evaluacion is None:
                    ...
                    try:
                        ...
                        if columnas_faltantes:
                            ...
                            # =================================================
                            # 11. FILTRAR SOLO ESTA EVALUACIÓN
                            # =================================================
                            ...
                            if df_preguntas.empty:
                                ...
                                # =================================================
                                # 14. GUARDAR PREGUNTAS EN MEMORIA
                                # =================================================
                                ...
                                # =================================================
                                # 20. CONTAR PREGUNTAS RESPONDIDAS
                                # =================================================
                                ...
                                # =================================================
                                # 22. FINALIZAR Y ENVIAR
                                # =================================================

                                if st.button(
                                    "Finalizar y enviar",
                                    key=(
                                        "boton_finalizar_evaluacion_general_"
                                        + evaluacion_id
                                    )
                                ):
                                    ...
                                    if (
                                        ...
                                        and any(
                                            ...
                                            for respuesta
                                            in respuestas_usuario.values()
                                        )
                                    ):

                                        st.warning(
                                            "En una evaluación nivel 2 "
                                            "debe seleccionar exactamente "
                                            "dos respuestas en cada pregunta."
                                        )
    ...
    if (
        ...
        if (
            ...
            if not preguntas_incorrectas:
                ...
                st.success(
                    "No tuviste respuestas incorrectas. "
                    "¡Excelente trabajo!"
                )
                ...
                st.write(
                    "Te equivocaste en las siguientes "
                    "preguntas:"
                )

                for item in preguntas_incorrectas:

                    st.error(
                        f"Pregunta {item['Numero']}: "
                        f"{item['Pregunta']}"
                    )
